...
class RedBlackTree:
    ...
    def print_tree(self):
        ...
        if self.root == self.NIL:
            ...
            return
        lines = []
        self._build_tree_string(self.root, 0, "ROOT:", lines)
        print("\nRed-Black Tree Structure:")
        print("\n".join(lines))
    
    def _build_tree_string(self, node, level, prefix, lines):
        """Recursively builds the tree string representation"""
        if node != self.NIL:
            color = "R" if node.color == 'red' else "B"
            lines.append(f"{' ' * level * 4}{prefix} {node.value}({color})")
            self._build_tree_string(node.left, level + 1, "L:", lines)
            self._build_tree_string(node.right, level + 1, "R:", lines)
...
# Print the tree in vertical format
rb_tree.print_tree()

print("\nBlack height:", rb_tree.get_black_height())  # Should return appropriate black height (likely 2)
print("Search for 20:", rb_tree.search(20))  # True
print("Search for 40:", rb_tree.search(40))  # False

# Delete a node and print the tree again
rb_tree.delete(20)
print("\nAfter deleting 20:")
rb_tree.print_tree()
print("Search for 20:", rb_tree.search(20))  # False
